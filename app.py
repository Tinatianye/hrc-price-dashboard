...
model_type = col[1].radio("Model Type for Japan", ["Joint VAR Model", "Japan Independent Model"])
...
fc_period = pd.date_range(start=final_df.index.tolist()[-1], end=final_df.index.tolist()[-1]+relativedelta(months=months-1), freq='MS')
...
if model_type == "Japan Independent Model" and "Japan" in country:
    jp_fc_df = pd.read_csv("JP_forecast.csv")
    jp_fc_df["Date"] = pd.to_datetime(jp_fc_df["Date"])
    jp_fc_df.set_index("Date", inplace=True)
    df_res["Japan HRC (FOB, $/t)_forecast"] = jp_fc_df["Japan HRC (FOB, $/t)_forecast"]
    japan_hrc = df_res.iloc[-1]["Japan HRC (FOB, $/t)_forecast"]

    def fun_japan_independent():
        d1 = final_df[[f'Japan HRC (FOB, $/t)']]
        d1.columns = [f"HRC (FOB, $/t)"]
        d1["t"] = f"Japan HRC (FOB, $/t)"
        d2 = jp_fc_df[[f"Japan HRC (FOB, $/t)_forecast"]]
        d2.columns = [f"HRC (FOB, $/t)"]
        d2["t"] = f"Japan HRC (FOB, $/t)_forecast"
        return pd.concat([d1, d2])

    d = [fun(i) for i in country if i != "Japan"]
    d.append(fun_japan_independent())
    d3 = pd.concat(d)
else:
    def fun(x):
        ...
        return pd.concat([d1, d2])
    d3 = pd.concat([fun(i) for i in country])
...
china_columns  =['HRC FOB China','Sea Freight','Basic Customs Duty (%)','LC charges & Port Charges','Exchange Rate (Rs/USD)','Freight from port to city']
...
japan_columns  =['HRC FOB Japan','Sea Freight','Basic Customs Duty (%)','LC charges & Port Charges','Exchange Rate (Rs/USD)','Freight from port to city']
