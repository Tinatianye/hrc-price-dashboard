...
# Load Japan forecast from CSV (external VAR output)
jp_fc = pd.read_csv("JP_forecast.csv")
jp_fc.rename(columns={"Unnamed: 0": "Date", "Japan HRC (FOB, $/t)_f": "Japan HRC (FOB, $/t)_forecast"}, inplace=True)
jp_fc["Date"] = pd.to_datetime(jp_fc["Date"])
jp_fc.set_index("Date", inplace=True)

# Replace Japan forecast if user selected it
if "Japan" in country:
    df_forecast_processed.update(jp_fc)




forecast_cols = [f"{c} HRC (FOB, $/t)_forecast" for c in country]
df_res = df_forecast_processed[forecast_cols]
