...
country = col1[2].multiselect("Please choose country", ["China", "Japan", "Korea"], ["China", "Japan", "Korea"])
...
df_res = df_forecast_processed[["China HRC (FOB, $/t)_forecast", "Japan HRC (FOB, $/t)_forecast", "Korea HRC (FOB, $/t)_forecast"]]
...
korea_df = pd.DataFrame(korea_data, index=korea_columns)
korea_df = korea_df.reset_index()
korea_df.columns=['Factors','Value']

korea_land_price = exchange_rate*(10+1.01*(korea_hrc+sea_freight)*(1+1.1*0))+500
...
if 'Korea' in country:
    col[0].write("**Korea**")
    col[0].dataframe(korea_df, hide_index=True)
    col[0].write('Korea land price is: '+str(round(korea_land_price)))
